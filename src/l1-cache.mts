...
const L1CacheFileName = ".l1.cache.json";
export const L1CacheState = {
  needsUpdate: false,
};
export const L1Cache: Record<string, Struct[]> = fs.existsSync(path.join(L1CacheFileName))
  ? Object.fromEntries(
      JSON.parse(fs.readFileSync(path.join(L1CacheFileName)).toString()).map(([k, v]: [string, any]) => [
...
export const onL1Finish = () => {
  ...
  return writeFile(
    path.join(L1CacheFileName),
    JSON.stringify(Object.entries(L1Cache).map(([k, v]) => [k.slice(baseCfgDir.length + 1), v.map((e) => e.toJson(true))])),
  );
};
